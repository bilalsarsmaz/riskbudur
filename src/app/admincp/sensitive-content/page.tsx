...
export default function SensitiveContentPage() {
    ...
    const rightSidebarContent = (
        <div className="rounded-2xl p-4 sticky top-4" style={{ backgroundColor: 'var(--app-surface)' }}>
            ...
            <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                    <p className="text-[13px]" style={{ color: 'var(--app-subtitle)' }}>Yasaklı Kelime</p>
                    <p className="text-2xl font-bold" style={{ color: 'var(--app-body-text)' }}>{words.length}</p>
                </div>
            </div>
    ...
    return (
        <AdmStandardPageLayout sidebarContent={<AdminSidebar />} rightSidebarContent={rightSidebarContent}>
            ...
            <div className="p-4">
                {/* Add Form */}
                <div className="bg-[var(--app-surface)] border border-theme-border rounded-2xl p-6 mb-6">
                    <h3 className="text-lg font-bold mb-4 flex items-center gap-2" style={{ color: 'var(--app-body-text)' }}>
                    ...
                    <form onSubmit={handleAdd} className="flex gap-3">
                        <input
                            ...
                            className="flex-1 px-4 py-3 bg-[var(--app-input-bg)] rounded-xl border-none focus:ring-2 focus:ring-[#DC5F00] text-[var(--app-body-text)]"
                        />
                        <button
                            type="submit"
                    ...
                    <p className="text-xs text-[var(--app-subtitle)] mt-2 ml-1">
                ...
                <div className="space-y-4">
                    <h3 className="text-lg font-bold px-1" style={{ color: 'var(--app-body-text)' }}>
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-12 bg-[var(--app-surface)] rounded-2xl border border-dashed border-theme-border">
                            <p className="text-[var(--app-subtitle)]">Henüz filtrelenen kelime yok.</p>
                        </div>
                    ) : (
                        <div className="flex flex-wrap gap-2">
                            {words.map((item) => (
                                <div
                                    key={item.id}
                                    className="group flex items-center gap-2 pl-4 pr-2 py-2 bg-[var(--app-surface)] border border-theme-border rounded-full hover:border-red-500/50 transition-colors"
                                >
                                    <span className="font-medium text-[var(--app-body-text)]">{item.word}</span>
                                    <button
                                        onClick={() => handleDelete(item.id, item.word)}
                                        className="p-1.5 rounded-full hover:bg-red-500/20 text-[var(--app-subtitle)] hover:text-red-500 transition-colors"
                                        title="Sil"
                                    >
                                        <IconTrash size={16} />
                                    </button>
