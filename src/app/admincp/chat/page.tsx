...
export default function AdminChatPage() {
    ...
    const rightSidebarContent = (
        <div className="rounded-2xl p-4 sticky top-4 bg-black border border-theme-border w-64">
            <h2 className="text-sm font-mono font-bold mb-4 text-green-500 uppercase tracking-wider border-b border-theme-border pb-2">
                ONLINE:
            </h2>
            <div className="space-y-1 text-sm font-mono text-theme-subtitle">
                {onlineUsers.map(u => (
                    <div key={u.nickname} className="hover:text-white cursor-pointer transition-colors">
                        <span>@{u.nickname}</span> <span className="text-theme-subtitle opacity-70">[{u.role.toLowerCase() === 'rootadmin' ? 'root' : u.role.toLowerCase() === 'moderator' ? 'mod' : 'admin'}]</span>
                    </div>
                ))}
                {onlineUsers.length === 0 && <span className="text-theme-subtitle italic">No active users</span>}
            </div>
        </div>
    );
    ...
    return (
        <AdmStandardPageLayout sidebarContent={<AdminSidebar />} rightSidebarContent={rightSidebarContent}>
            ...
            <div className="flex flex-col h-[calc(100vh-80px)] p-2">
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-1">
                            ...
                            {messages.map((msg) => {
                                ...
                                return (
                                    <div key={msg.id} className="break-words hover:bg-theme-hover px-1 -mx-1 rounded">
                                        <span className="text-theme-subtitle mr-2">[{time}]</span>
                                        ...
                                        <span className="text-theme-subtitle mx-1">@{roleDomain}:</span>
                                        <span className="text-theme-text">{msg.content}</span>
                                    </div>
                                );
                            })}
                ...
                {/* Input Area */}
                <div className="bg-theme-surface border border-t-0 border-theme-border rounded-b-lg p-3">
                    <form onSubmit={handleSend} className="flex gap-2 items-center">
                        ...
                        <input
                            ...
                            autoFocus
                        />
                        <button type="submit" disabled={sending} className="text-theme-subtitle hover:text-green-500 transition-colors">
                            <IconSend size={18} />
                        </button>
                    </form>
                </div>
